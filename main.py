...
for image_name in image_names:
    for row, col in schema:
        ...
        response_lidar = requests.request("GET", url_lidar, headers=headers, data=payload)

        # Check if the request was successful
        if response_raw.status_code == 200 and response_lidar.status_code == 200:
            # Read the image from the response
            image_data_raw = BytesIO(response_raw.content)
            image_data_lidar = BytesIO(response_lidar.content)

            # Open the image using PIL
            image_raw = Image.open(image_data_raw)
            image_lidar = Image.open(image_data_lidar)

            # Save the image
            image_raw.save(f"data/{image_name}_raw_{row}_{col}.jpg")  # Save with appropriate name
            image_lidar.save(f"data/{image_name}_lidar_{row}_{col}.jpg")  # Save with appropriate name
            ...
            print(f"Failed to retrieve the response_lidar at ({row},{col}). Status code:", response_lidar.status_code)
